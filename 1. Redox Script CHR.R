...
summary_table_wide <- summary_table_wide %>% mutate(
  ...
  miRNA_snb = miRNA / 0.22
)
write.csv(summary_table_wide, "net_benefit_summary_table.csv", row.names = FALSE)
...
ggplot(data = dca_all, aes(x = threshold, y = net_benefit, color = label)) +
  ...
  theme_classic()
ggsave("dca_all_summary_170325.png", width = 20, height = 15, scale = 0.3)
