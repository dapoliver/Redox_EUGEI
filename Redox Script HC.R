...
write.csv(coef_results, "logistic_regression_LASSO_coefficients_hc.csv", row.names = FALSE)
write.csv(all_predictions, "predictions_output_hc.csv", row.names = FALSE)

predictions_data = read.csv("predictions_output_hc.csv") # Read the predictions output file
...
for (set in predictor_sets) {
  ...
  ggsave(paste0("calibration_plot_", set, "_hc.png"), plot = cal_plot, width = 6, height = 5, dpi = 300)
  ...
  write.csv(dca_assessment, paste0("dca_summary", set,"_hc.csv"), row.names = FALSE)
...
##### DCA summary #####
dca_b <- read_csv("dca_summarya_hc.csv")
dca_d <- read_csv("dca_summaryc_hc.csv")
...
ggplot(data=dca_all, aes(x = threshold, y = net_benefit, color = label)) +
  ...
  coord_cartesian(ylim = c(-0.1, 0.075)) +
  ...
  theme_classic()
ggsave("dca_all_summary_hc.png", width=20, height=15, scale=0.5)
