...
# Create leave one site out cross validation
outer_folds <-
...
temp_results <- data.frame(
  ...
  LR_neg = numeric(),
  brier = numeric(),
  cal_intercept = numeric(),
  cal_slope = numeric(),
  stringsAsFactors = FALSE
)

# Loop over each outer fold
for (fold_num in sites) {
  train <- data %>% filter(site != fold_num)
  test <- data %>% filter(site == fold_num)
  ...
  lr_pos <- tryCatch(posLr(obs = observed, pred = predicted_labels, pos_level = 1)$posLr,
    error = function(e) {
      warning("posLr failed: ", e$message)
      NA_real_
    }
  )
  lr_neg <- tryCatch(negLr(obs = observed, pred = predicted_labels, pos_level = 1)$negLr,
    error = function(e) {
      warning("negLr failed: ", e$message)
      NA_real_
    }
  )
  ...
  temp_results <- rbind(temp_results, data.frame(
    fold_num = fold_num,
    train_n = nrow(train),
    train_events = sum(train$Transition_status == "T"),
    test_n = nrow(test),
    test_events = sum(test$Transition_status == "T"),
    ...
    LR_pos = lr_pos,
    LR_neg = lr_neg,
    brier = logistic_calibration$BrierScore[1],
    cal_intercept = logistic_calibration$CalInt[1],
    cal_slope = logistic_calibration$CalSlope[1]
  ))
  # Save predictions
...
write_csv(results_new, "CV_results_losocv.csv")
write_csv(temp_results, "CV_results_losocv_site_level.csv")
...
write.csv(results_NAPLS, "external_validation_results_losocv.csv", row.names = FALSE)
...
write.csv(results_NAPLS_recal, "external_validation_results_recal_losocv.csv", row.names = FALSE)
