...
for (fold_num in seq_along(outer_folds)) {
  ...
  for (var in predictors[[1]]) {
    model <- lm(train_corrected[[var]] ~ Age + Gender + Ethnicity, data = train_corrected)

    models[[var]] <- model # store model

    train_corrected[[paste0(var, "_resid")]] <- residuals(model)
  }
  ...
  for (var in predictors[[1]]) {
    ...
    preds <- predict(model, newdata = test_corrected)

    test_corrected[[paste0(var, "_resid")]] <- test_corrected[[var]] - preds
  }
...
write.csv(coef_results, "Results/logistic_regression_LASSO_coefficients_110325.csv", row.names = FALSE)
...
results_new
write_csv(results_new, "Results/CV_results_mean_offset_170326.csv")
...
results_NAPLS
write.csv(results_NAPLS, "Results/external_validation_results_mean_offset_170326.csv", row.names = FALSE)
...
write.csv(results_NAPLS_recal, "Results/external_validation_results_recal_mean_offset_121225.csv", row.names = FALSE)
...
write.csv(summary_table_wide, "net_benefit_summary_table_210326.csv", row.names = FALSE)
...
ggplot(data = dca_all, aes(x = threshold, y = net_benefit, color = label)) +
  ...
  theme_classic()
ggsave("dca_all_summary_210326.png", width = 20, height = 15, scale = 0.3)
